      ...
      promiseTOReadJDContent.then(function(JDcontent){
        ...
        promiseToGetResumekeyphrases.then(function(resumephrase){
          ...
          promiseToGetJDkeyphrases.then(function(JDphrase){
            ...
            promiseToGetJDintent.then(function(JDintent){
              JDintents = JDintent;
              console.log("Inside main JDintents",JDintents,JDintents.length);
              let promiseToGetresumeintent =  getIntents1(resumephrase,[],[],phrasecount);
              promiseToGetresumeintent.then(function(resumeintent){
                resumeintents = resumeintent;
                console.log("Inside main resumeintents",resumeintents,resumeintents.length);
              }).catch(function (error) {  
                console.log("Error in Getting resumeintents is",error.message); 
              });
            }).catch(function (error) {  
              console.log("Error in Getting JD Intents is",error.message); 
            });
           // let resumeintent = getIntents(resumephrase,[],[],phrasecount);  
           // console.log("Inside main JDintent",JDintent,JDintent.length);
           // console.log("Inside main resumeintent",resumeintent,resumeintent.length);
          }).catch(function (error) {  
...
{ 
  ...
  var ajaxPromises =  luisurllist.map(
    ...
    Promise.all(ajaxPromises)
      ...
      // console.log("Promise all executed",luisintent);
       // console.log("First Promise all executed",luisintent[0]);
        console.log("Second Promise all executed",luisintent[0],luisintent[0].length);
    ...
    //  getIntents(errorphrases,luisintent,[],errorphraselength);
       setTimeout(getIntents,6000,errorphrases,luisintent,[],errorphraselength);
    });
  });
}

function getIntents1(resumekeyphrase,luisintent,errorphrases,count)
{ 
