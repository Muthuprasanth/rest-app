...
var url = require('url');;
var phrasecount = 5;
var globresumeintentsarray = [], globJDintentsarray = [];
exports.list_all_tasks = async function (req, res) {
  ...
  promiseTOGetsendgridCredentials.then(function (Credentials) {
    ...
    promiseTOReadResumeContent.then(function (resumecontent) {
      ...
      promiseTOReadJDContent.then(function (JDcontent) {
        ...
        //  let promiseToGetJDintent = getIntents(JDphrase, [], [], phrasecount);
             let promiseToGetJDintent  =  helperToGetIntents(JDphrase,globresumeintentsarray);
            promiseToGetJDintent.then(function (JDintent) {
              console.log("Inside main JDintent", JDintent);
            //  console.log("Inside main JDintent", JDintent, JDintent.length);
             ...
             //let JDi = await helperToGetIntents(JDphrase,globresumeintentsarray);
             //console.log("JDIntents from helper",JDi);
          }).catch(function (error) {
 ...
 function helperToGetIntents(phrases,globintentsarray)
{
  return new Promise(function(resolve, reject) {
    let promiseToGetIntents = getIntents(phrases,globintentsarray,[],phrasecount);
    promiseToGetIntents.then(function(val){
      console.log("Inside helperToGetIntents from promise",val[0]);
       resolve(val[0]);
    }).catch(function (errorphrases) {  
      console.log("Error in Getting JD Intents is",errorphrases); 
      console.log("globresumeintentsarray "+globresumeintentsarray+" ");
      setTimeout(getIntents,1500,errorphrases,globresumeintentsarray,[],errorphrases.length);
    });
...
function getIntents(resumekeyphrase, luisintent, errorphrases, count) {
  ...
  /*  let p = Promise.all(ajaxPromises).catch(function (errorphrases) {
      ...
      setTimeout(getIntents, 1500, errorphrases, luisintent, [], errorphraselength, intentsarray);
    });*/
