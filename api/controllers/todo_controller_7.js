      ...
      promiseTOReadJDContent.then(function(JDcontent){
        ...
        promiseToGetResumekeyphrases.then(function(resumephrase){
          ...
          promiseToGetJDkeyphrases.then(function(JDphrase){
            ...
            comparekeyphrases(resumephrase,JDphrase);   
...
function comparekeyphrases(resumekeyphrase,jdkeyphrase)
{ 
  ...
  console.log("lenght of the keyphrases before ",resumelength,jdlength);

var count =resumelength;
var options4 ={};
let luisurl ="";
var luisintent = [];
var errorphrases = [];
var index = 0,errorindex=0;
for(var x=0;x<count;x++){
    ...
    requestPromise(options4).then(function(body){
      ...
      console.log("Inside promiseTOCallLuis result from LUIS",body);
      luisintent[index]=body_.topScoringIntent.intent;  
      index++;
      ...
      if(index>=count)
      {
         console.log("index is"+index+"luis intent array",luisintent);
      }
    }).catch(function (err) {
      console.log("Error occurred Inside promiseTOCallLuis",err.message);
      errorphrases[errorindex]=body_.query;  
      errorindex++;
      index++;
      luisintent[index] = "error";
      index++;
      if(index>=count)
      {
         console.log("index is"+index+"luis intent array",luisintent);
